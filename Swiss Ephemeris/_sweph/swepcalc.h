...
/*************************************************************
 ...
 julian days.
*************************************************************/
int nacalc(double jd_ad, centisec *plon, centisec *pspe);
int calcserv(int id, double t, int flag, int plalist, char *so);
void helup(double jd_ad);
void togeo(double le, double re, double l, double r, double z, double *alg, double *arg);
int  calc(int p,
		 double jd_ad,
		 int flag, 
		 double *alng,
		 ...
		 double *alat,
		 double *alngspeed);
int rel_geo(int p, double rau);
int hel( int	p,	/* planet index as defined by placalc.h */
...
char *placalc_set_ephepath(char *new_path);	/* sets ephepath;
...
void revjul (double u, int gregflag,
	      int *jmon, int *jday, int *jyear, double *jut);
void revjuls(double u, int gregflag, ADATE *adp); 
...
int InpHouseNr(HOUSES *h, CSEC p, CSEC *cuspoff);
   ...
   */
int InpHouseNr2(HOUSES *h, CSEC p, CSEC *cuspoff);
  /* variation of InpHouseNr(). Comment in swepcalc.c */

void CalcHouses(CSEC th, CSEC fi, CSEC ekl, char hsy, int icnt,
	struct houses *h);
